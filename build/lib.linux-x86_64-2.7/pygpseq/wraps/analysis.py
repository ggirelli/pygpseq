...
class Analyzer(iot.IOinterface):
	...
	Args:
		...
		nsf (tuple[int]): nuclear selection features according to pygpseq.const.
			Uses up to three features. The first two features are used in the
			summary scattersplot (i.e., order matters).
		...
		cdescr (dict): dictionary with better condition descriptions. The keys
			are the condition subfolder names, the values the descriptions.
	"""
