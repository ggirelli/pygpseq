...
""" Functions for chromatic aberration correction """
...
def dot_candidates(I,
	...
	@return:
	A record array with the following field:
	...
	# Prepare output
	P = np.zeros((len(Pos[0]),),
		dtype = [('dog', 'float'), ('int', 'float'),
		('x', 'u4'), ('y', 'u4'), ('z', 'u4')])
	P['dog'] = DoG[Pos]
	P['int'] = I[Pos]
	P['x'] = Pos[0]
...
def set_borders(I, paddings = None , value = None):
	...
	for j in range(len(I.shape)):
		...
		# Right
		right_pad = [tuple(range(I.shape[i])) if i != j
			else tuple(range(p))
			for i in range(len(I.shape))]
		J[np.ix_(*right_pad)] = value

		# Left
		left_pad = [tuple(range(I.shape[i])) if i != j
...
def correct_stack(stack, chname, m):
	...
	@param:
	 ...
	 - m <{cnames, P:ndarray, N:int}}> CA correction measurements
	"""
