...
def export_profiles(profiles, path):
	""" Exports the profiles to a csv file. """

	nsteps = len(profiles[0]['ratio']['x'])
	nrows = nsteps * len(profiles)

	outcsv = np.zeros((nrows,), dtype = const.DTYPE_PROFILE_EXPORT)
	...
	for profile in profiles:
		...
		outcsv[c:(c+nsteps),] = subcsv
		c += nsteps

	# Export condition summary
	np.savetxt(path, outcsv, delimiter = ',', comments = '', fmt = [
		'%s',    '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e',
		'%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e',
		'%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e', '%.18e',
		'%.18e', '%.18e', '%.18e'],
		header = ",".join([h for h in outcsv.dtype.names]))
	return(outcsv)
